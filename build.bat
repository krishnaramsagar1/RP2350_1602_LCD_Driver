...
arm-none-eabi-as -g -mcpu=cortex-m33 -mthumb 1602_lcd.s -o 1602_lcd.o
if errorlevel 1 goto error
...
REM ==============================================================================
arm-none-eabi-ld -g -T linker.ld vector_table.o reset_handler.o stack.o xosc.o reset.o coprocessor.o uart_module.o gpio.o delay.o 1602_lcd.o main.o image_def.o -o 1602_lcd.elf
if errorlevel 1 goto error

REM ==============================================================================
REM Create Raw Binary from ELF
REM ==============================================================================
arm-none-eabi-objcopy -O binary 1602_lcd.elf 1602_lcd.bin
if errorlevel 1 goto error
...
REM ==============================================================================
python uf2conv.py -b 0x10000000 -f 0xe48bff59 -o 1602_lcd.uf2 1602_lcd.bin
if errorlevel 1 goto error
...
echo.
echo =================================
echo SUCCESS! Created 1602_lcd.uf2
echo =================================
echo.
echo To flash via UF2:
echo   1. Hold BOOTSEL button
echo   2. Plug in USB
echo   3. Copy 1602_lcd.uf2 to RP2350 drive
echo.
echo To flash via OpenOCD (debug probe):
echo   openocd -f interface/cmsis-dap.cfg -f target/rp2350.cfg -c "adapter speed 5000" -c "program 1602_lcd.elf verify reset exit"
echo.
goto end
